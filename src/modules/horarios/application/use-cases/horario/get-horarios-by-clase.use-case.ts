// filepath: g:\PROYECTOS\COLLEGE\BACK\src\modules\horarios\application\use-cases\horario\get-horarios-by-clase.use-case.ts
...
@Injectable()
export class GetHorariosByClaseUseCase {
  constructor(
    @Inject(HORARIO_REPOSITORY)
    private readonly horarioRepository: HorarioRepositoryInterface,
  ) {}

  async execute(claseId: number, institucionId: number): Promise<Horario[]> {
    return this.horarioRepository.findByClase(claseId, institucionId);
  }
}
